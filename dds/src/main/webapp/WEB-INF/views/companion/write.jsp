...
function updateSigungu(areaCode) {
	$('select[name=sigunguCode]').html('');
	let arr;
	switch(areaCode) {
		case "서울" : {
	...
	let newArea=$("<select name='sigunguCode'></select>");
	
	if(arr!=null) {
		for(let sigungu of arr) {
			let opt=document.createElement('option');
			opt.text=sigungu;
			opt.value=sigungu;
			$('select[name=sigunguCode]').append(opt);
		}
	}
	$('select[name=sigunguCode]').prepend("<option selected>선택</option>");
}
...
$(function(){
	$('select[name=areaCode]').change(function(){
		let areaCode=$('select[name=areaCode] option:selected').val();
		
		$('select[name=sigunguCode]').html('');
		
		updateSigungu(areaCode);
	});
	
	$('.addRegion').click(function(){
		let areaCode=$('select[name=areaCode] option:selected').val();
		let newArea=$("<select name='sigunguCode'></select>");
		newArea.append("<option selected>선택</option>");
